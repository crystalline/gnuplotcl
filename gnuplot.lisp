...
(def-gp-ext-method lplot proc (data)
			...
			(labels ((rec (data acc comma)
              ...
              ;Convert arrays to lists of any
              (let ((data (if (some #'arrayp data)
                              (mapcar (lambda (x) (if (and (arrayp x)
                                                           (eq (length (array-dimensions x)) 1))
                                                      (map 'list (lambda (val) val) x)
                                                      x))
                                      data)
                            data)))
                  ...
                  (mapcar #'(lambda (data)
                              (let ((data (apply #'mapcar (cons #'list data))))
                                (mapcar #'(lambda (&rest lambda-args)
                                            (mapcar #'(lambda (val)
                                        ;Choose format depending on element types
                                                        (let ((fmt (if (some #'floatp val)
                                                                       "~{~F ~}"
                                                                     "~{~A ~}")))
                                                          (gp proc 'gp-princ (format nil fmt val)) ;Changed to float representation to avoid conflict with CL default double float printing format
                                                          (gp proc 'gp-flush)))
                                                    lambda-args))
                                        data))
                              (gp proc 'gp-princ "e")
